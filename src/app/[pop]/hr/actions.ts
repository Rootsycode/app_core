...
> {
  try {
    ...
    let ownerProfile = { first_name: '', last_name: '', image_url: null as string | null }
    ...
    const members: MemberRow[] = (uprRows || []).map((row) => {
      const rel = row.roles as { name: string; display_name: string } | null
    ...
    if (owner) {
      ...
      if (!invErr && inv) {
        pendingInvites = inv.map((i) => {
          const rr = i.roles as { display_name: string } | null
          return {
            id: i.id,
            email: i.email,
            roleId: i.role_id,
...
> {
  ...
  if (sameUserId(user.uid, inviteeId)) {
  ...
  if (
    roleRow.pop_id != null &&
    String(roleRow.pop_id) !== String(popId)
  ) {
...
export async function deactivatePopMember (
  ...
  if (ownerUserId && sameUserId(memberUserId, ownerUserId)) {
    return { success: false, error: 'No se puede quitar al propietario del POP.' }
  }
...
> {
  ...
  const ownerUserId =
    'ownerUserId' in popRes.pop ? popRes.pop.ownerUserId : null
  if (!(await isPopOwner(popId, user.uid, ownerUserId ?? null))) {
    return { success: false, error: 'Solo el dueño puede editar permisos de roles.' }
  }
  ...
  const selectedPermissionIds = (rpRows || []).map((r) => String(r.permission_id))
